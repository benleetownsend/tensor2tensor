...
def adv_transformer_internal(inputs, targets, target_space, hparams):
  ...
  with tf.variable_scope("adv_transformer"):
    ...
    intermediate = tf.constant(34*1024 - 1, dtype=targets.dtype)
    ...
    if hparams.mode != tf.estimator.ModeKeys.TRAIN:
      extra_step_prob = 1.0
    for _ in range(hparams.extra_steps):
    ...
    alpha = tf.random_uniform(
                      shape=[tf.shape(targets_emb_noisy)[0],1,1,1],
                      minval=0.,
                      maxval=1.)
    
    differences = tf.stop_gradient(res_emb - targets_emb_noisy)
    interpolates = tf.stop_gradient(targets_emb_noisy + (alpha*differences))
    gradients = tf.gradients(adversary(interpolates,
                                       tf.stop_gradient(inputs + inputs_emb),
                                       hparams, "adversary", reuse=True), [interpolates])[0]
    slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients), reduction_indices=[1,2,3]))
    gradient_penalty = tf.reduce_mean((slopes-1.)**2)
    
    losses["lipschitz-penalty"] = gradient_penalty * 10
...
class TransformerAdv(t2t_model.T2TModel):
  ...
  def infer(self, features=None, decode_length=50, beam_size=1, top_beams=1,
    ...
    if "partial_targets" in features:
      ...
      length = tf.Print(length, [length])
      initial_output = tf.zeros((batch_size, 2 * length, 1, 1), dtype=tf.int32)
    ...
    samples = tf.Print(samples, [samples[0]])
    # More steps.
    how_many_more_steps = 0
