# Based on the ngram model from an old version of NLTK, modified and fixed by Ben Townsend
...
class NgramModel:
  ...
  def __init__(self, n, train, pad_left=True, pad_right=False,
    ...
    for i, sent in tqdm(enumerate(train()), desc="Fitting {}-gram model".format(n)):
      ...
      if i % 100000 == 0:
        cfd, self._ngrams = prune(cfd, self._ngrams)
...
class LangModel:
  ...
  def fit(self, data):
    self.lm = NgramModel(6, data, estimator=cpdist)
