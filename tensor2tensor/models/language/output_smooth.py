...
def biased_beam_search_decoder(logits, predict_next, beam_size, alpha=1.7):
  ...
  for t, step_logits in enumerate(logits):
    ...
    top_samples = np.unravel_index(
      np.argpartition((samples / calc_len_pen(beams, alpha)[:, None]).ravel(), -curr_num_beams)[-curr_num_beams:],
      samples.shape)
    new_beams = []
    ...
    try:
      top_final_beams = np.argpartition(log_probs / calc_len_pen(beams, alpha, close_beams=True), -beam_size)[
                        -beam_size:]
    except ValueError:
  ...
  for beam, prob in zip(beams, log_probs):
    if beam_is_finished(beam):
      ...
      beam = beam[
             :logit_len - 1]  # if the sequence is too long, trim off the last token - this is an edge case and should never happen.
      output_beams.append(beam + [_EOS])
      output_probs.append(
        prob + predict_next(beam, _EOS))  # Force an EOS token on the end and recalculate the probabilities
